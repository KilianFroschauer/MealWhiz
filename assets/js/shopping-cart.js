...
(function (ShoppingCart) {
                ...
                if (items.length === 0) {
                    ...
                    document.querySelectorAll(".decrease-qty").forEach((btn) => {
                        btn.addEventListener("click", function () {
                            ...
                            if (!ingredient)
                                return;
                            const input = document.querySelector(`.item-quantity[data-ingredient="${ingredient}"]`);
                            if (!input)
                                return;
    ...
    async function deleteFromCart(ingredient) {
        const token = localStorage.getItem("accessToken");
        if (!token) {
    ...
    async function updateQuantity(ingredient, quantityStr) {
        const token = localStorage.getItem("accessToken");
        if (!token) {
        ...
        if (isNaN(quantity) || quantity < 1) {
            ...
            const inputElement = document.querySelector(`.item-quantity[data-ingredient="${ingredient}"]`);
            if (inputElement) {
        ...
        try {
            ...
            if (getRes.ok) {
                ...
                const item = items.find((i) => i.ingredients === ingredient);
                if (item) {
    ...
    window.deleteAllFromCart = deleteAllFromCart;
