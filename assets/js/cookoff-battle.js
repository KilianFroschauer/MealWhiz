...
function initializeBattleUI(settings) {
    ...
    if (settings.mode === 'casual') {
        if (casualContent)
            casualContent.classList.remove('d-none');
        if (competitiveContent)
            competitiveContent.classList.add('d-none');
        initializeCasualMode();
    }
    else {
        if (casualContent)
            casualContent.classList.add('d-none');
        if (competitiveContent)
            competitiveContent.classList.remove('d-none');
...
function initializeCompetitiveMode(settings) {
...
function setupCasualChat() {
    const chatInput = document.querySelector('#casualModeContent .chat-input');
...
function sendCasualMessage(input, container) {
...
function simulatePartnerResponse(container) {
    const responses = [
...
function updateCompetitionDetails(settings) {
    ...
    if (challengeName) {
        ...
        switch (settings.challenge) {
            case 'breakfast-bowl':
                displayName = "Breakfast Bowl Challenge";
                break;
            case 'poke-bowl':
                displayName = "Poke Bowl Showdown";
                break;
            case 'seafood':
                displayName = "Seafood Extravaganza";
                break;
            case 'pasta-perfection':
                displayName = "Pasta Perfection Battle";
                break;
            case 'dessert-duel':
                displayName = "Sweet Dessert Duel";
                break;
            case 'veggie-victory':
                displayName = "Vegetarian Victory Challenge";
                break;
            case 'quick-meal':
                displayName = "30-Minute Meal Masterpiece";
                break;
            case 'mystery-box':
...
function setupSpectatorChat() {
    const chatInput = document.querySelector('#competitiveModeContent .spectator-chat-input');
...
function sendSpectatorMessage(input, container) {
    const message = input.value.trim();
    if (message) {
