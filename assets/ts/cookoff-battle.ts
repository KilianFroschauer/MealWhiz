namespace CookOffBattle {
    ...
    const JITSI_SERVER_URL: string = (typeof MealWhizConfig !== 'undefined' ?
    ...
    function initializeJitsi(settings: BattleSettings, userName: string) {
        ...
        console.log(
            `Initializing Jitsi Meet with room: "${roomNameForJitsi}", user: "${userName}", role: "${settings.role || "competitor"
            }"`
        );
    ...
    function initJitsiMeetExternalAPI(
        roomName: string,
        displayName: string,
        ...
        messageDiv.innerHTML = `
            <div class="d-flex ${isCurrentUserMsg ? "flex-row-reverse" : "flex-row"} mb-2">
                ...
                : message.type === "system"
                    ...
                    <small class="message-timestamp text-muted d-block ${isCurrentUserMsg ? "text-end" : "text-start"
