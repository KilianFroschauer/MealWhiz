...
document.addEventListener('DOMContentLoaded', (): void => {
    ...
    function promptToJoinLobby(lobbyId: string, lobbyName: string, streamUrl: string): void {
        ...
        confirmationBody.textContent = `Are you sure you want to join lobby "${lobbyName}"?`;        const handleYesClick = async () => {
    ...
    async function fetchAndRenderPublicLobbies(container: HTMLElement): Promise<void> {
        ...
        try {
            ...
            ul.className = 'list-group';            lobbies.forEach(lobby => {
...
// --- COMPETITIVE MODE FUNCTIONS ---
    ...
    async function fetchCompetitiveLobbies(container: HTMLElement): Promise<void> {
        ...
        try {
            ...
            const lobbies: Array<{ id: string; challengeType?: string; hostName?: string; streamUrl: string; participants: any[] }> = await response.json();
    ...
    (window as any).joinCompetitiveLobby = async function(lobbyId: string, lobbyName: string, streamUrl: string, role: 'competitor' | 'viewer') {
        try {
            const token = localStorage.getItem('accessToken');
            if (!token) {
                alert("Please log in to join a lobby.");
