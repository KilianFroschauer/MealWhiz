...
namespace CookOff {
    ...
    document.addEventListener("DOMContentLoaded", (): void => {
        ...
        async function handleCreateCasualLobbyClick(): Promise<void> {
            const form = document.getElementById("casualLobbyForm") as HTMLFormElement;
            const titleInput = document.getElementById("casualLobbyTitle") as HTMLInputElement;
            ...
            try {
                ...
                window.location.href = `CookOff-Battle.html?mode=casual&eventId=${eventId}&streamUrl=${encodeURIComponent(
                    streamUrl
        ...
        function promptToJoinLobby(lobbyId: string, lobbyName: string, streamUrl: string): void {
            ...
            const handleYesClick = async () => {
                ...
                try {
                    ...
                    hideBootstrapModal(findingModalElement);
                    // Pass username to Battle page for Jitsi
                    window.location.href = `CookOff-Battle.html?mode=casual&eventId=${lobbyId}&streamUrl=${encodeURIComponent(
                        streamUrl
                    )}&username=${encodeURIComponent(userInfo.username)}`;
                } catch (error) {
        ...
        async function fetchAndRenderPublicLobbies(container: HTMLElement): Promise<void> {
            ...
            try {
                ...
                const response = await fetch(`${apiBase}/events?mode=casual&status=open&t=${timestamp}`); // Adjust API endpoint if needed
                if (!response.ok) {
                    throw new Error(`Failed to fetch lobbies: ${response.statusText} (${response.status})`);
                }
                const lobbies: Array<{ id: string; name?: string; streamUrl: string; [key: string]: any }> =
                    await response.json();
                ...
                container.innerHTML =
                    '<p class="text-center text-danger">Could not load public lobbies. Please try again later.</p>';
            }
        }
        ...
        function setupLobbyInterface(modalBody: HTMLElement): void {
            ...
            createLobbyBtn.innerHTML =
                '<span style="font-size: 1.2em; line-height: 1; vertical-align: middle; margin-right: 0.3em;">+</span> Create Lobby';
        ...
        if (startCasualCookingBtn) {
            startCasualCookingBtn.addEventListener("click", function (): void {
            ...
            startCompetitionBtn.addEventListener("click", async function (): Promise<void> {
                ...
                if (competitiveModalElement && findingOpponentElement && challengeTypeEl && difficultyLevelEl) {
                    ...
                    try {
                        ...
                        window.location.href = `CookOff-Battle.html?mode=competitive&eventId=${eventId}&streamUrl=${encodeURIComponent(
                            streamUrl
                        )}`;
                    } catch (error) {
        ...
        if (createCompetitiveLobbyBtn) {
            createCompetitiveLobbyBtn.addEventListener("click", () => {
                ...
                if (createModal) showBootstrapModal(createModal);
            });
        }
    ...
    async function handleCreateCompetitiveLobbyClick(): Promise<void> {
        const form = document.getElementById("competitiveLobbyForm") as HTMLFormElement;
        const durationInput = document.getElementById("cookoffDuration") as HTMLInputElement;
        const descriptionInput = document.getElementById("cookoffDescription") as HTMLInputElement;
        ...
        try {
            ...
            if (createModal) hideBootstrapModal(createModal);
            if (competitiveModal) hideBootstrapModal(competitiveModal);

            // Redirect to battle page with competitive mode and duration
            window.location.href = `CookOff-Battle.html?mode=competitive&eventId=${eventId}&streamUrl=${encodeURIComponent(
                streamUrl
            )}&username=${encodeURIComponent(userInfo.username)}&duration=${duration}&description=${encodeURIComponent(
                description
            )}`;
        } catch (error) {
    ...
    function setupCompetitiveLobbies(): void {
        const competitiveLobbiesList = document.getElementById("competitiveLobbiesList");
        if (!competitiveLobbiesList) return;

        // Fetch competitive lobbies
        fetchCompetitiveLobbies(competitiveLobbiesList);
    }

    async function fetchCompetitiveLobbies(container: HTMLElement): Promise<void> {
        container.innerHTML = '<p class="text-center">Loading competitive lobbies...</p>';
        try {
            const timestamp = new Date().getTime();
            const response = await fetch(`${apiBase}/events?mode=competitive&status=pending&t=${timestamp}`);
            if (!response.ok) {
                ...
                item.innerHTML = `
                        <div class="d-flex justify-content-between align-items-center">                        <div>
                            ...
                            <div>
                                ${
                                    isOpen
                                        ? `<button class="btn btn-sm btn-danger me-2" onclick="joinCompetitiveLobby('${
                                              lobby.id
                                          }', '${lobby.challengeType || "CookOff"}', '${
                                              lobby.streamUrl
                                          }', 'competitor')">Join as Competitor</button>
                                     <button class="btn btn-sm btn-outline-primary" onclick="joinCompetitiveLobby('${
                                         lobby.id
                                     }', '${lobby.challengeType || "CookOff"}', '${
                                              lobby.streamUrl
                                          }', 'viewer')">Join as Viewer</button>`
                                        : `<button class="btn btn-sm btn-outline-primary" onclick="joinCompetitiveLobby('${
                                              lobby.id
                                          }', '${lobby.challengeType || "CookOff"}', '${
                                              lobby.streamUrl
    ...
    (window as any).joinCompetitiveLobby = async function (
        lobbyId: string,
        lobbyName: string,
        streamUrl: string,
        role: "competitor" | "viewer"
    ) {
        try {
            const token = localStorage.getItem("accessToken");
            if (!token) {
            ...
            window.location.href = `CookOff-Battle.html?mode=competitive&eventId=${lobbyId}&streamUrl=${encodeURIComponent(
                streamUrl
            )}&username=${encodeURIComponent(userInfo.username)}&role=${role}`;
        } catch (error) {
