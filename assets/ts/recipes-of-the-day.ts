...
namespace RecipesOfTheDay {
    ...
    // Declare bootstrap as a global variable to inform TypeScript about its existence.
    // This is necessary if Bootstrap's JavaScript is loaded via a <script> tag and not as a module.
    ...
    document.addEventListener("DOMContentLoaded", async function () {
        ...
        try {
            ...
            const res = await fetch(`${apiBase}/recipes?minRating=4.5`); // Fetch a bit more to ensure variety if some images fail
            if (!res.ok) {
            ...
            recipes = recipes.sort((a: any, b: any) => (Number(b.ratings) || 0) - (Number(a.ratings) || 0)).slice(0, 3);

            if (!recipes.length) {
                // If no recipes meet the criteria, display an informational message.
                carouselInner.innerHTML =
                    '<div class="alert alert-info m-0 text-center">No recipes of the day found. Check back later!</div>';
                return;
            }
            ...
            carouselInner.innerHTML =
                '<div class="alert alert-danger m-0 text-center">Could not load recipes of the day. Please try again later.</div>';
        }
    });
}
